  ...
  return (
    <section className="grid gap-8 pb-4 md:grid-cols-[minmax(0,1fr)_22rem] md:pb-6">
      <div>
        ...
        <p className="mt-4 max-w-2xl text-[length:var(--text-body-lg)] leading-[var(--leading-body)] text-muted-foreground">
        ...
        <p className="mt-4 max-w-2xl border-l-2 border-secondary/40 pl-4 text-sm leading-7 text-teal">
      ...
      <aside className="rounded-lg border border-border bg-surface p-4">
        ...
        <div className="mb-4 grid grid-cols-3 gap-2">
          {evidenceStats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-lg border border-border bg-surface-2 px-2 py-3 text-center"
            >
...
function ProblemSection() {
  return (
    <section className="border-t border-border bg-surface-2 py-8 md:py-12">
        ...
        {processSteps.map((step, index) => (
          <article key={step.step} className="relative">
            {index < processSteps.length - 1 ? (
              <div className="absolute top-6 left-full hidden h-px w-[calc(100%-2rem)] bg-border xl:block" />
            ) : null}
            <div className="relative h-full rounded-lg border border-border bg-surface p-4">
    ...
    >
      <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_20rem]">
        <div>
          ...
          <h2 className="mt-2 max-w-3xl text-balance font-display text-[var(--text-h2)] leading-[var(--leading-heading)] tracking-[var(--tracking-heading)] text-foreground">
...
function FinalCallToAction() {
  return (
    <section className="py-8 md:py-14">
      <div className="rounded-lg border border-secondary/30 bg-teal-dim px-6 py-8 text-center">
        <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-teal">
          Marketplace entry
        </p>
